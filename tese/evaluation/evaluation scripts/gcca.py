...
path_mfcc = "/home/joao/Documents/tese/Tese/tese/evaluation/evaluation scripts/qm_featuresMFCC.csv"
path_chroma = "/home/joao/Documents/tese/Tese/tese/evaluation/evaluation scripts/qm_featuresChroma.csv"
viewMFCC = np.transpose(np.loadtxt(path_mfcc, delimiter=','))
viewChroma = np.transpose(np.loadtxt(path_chroma, delimiter=','))

data_train = []
data_train.append(viewMFCC)
data_train.append(viewChroma)
data_test = []
data_test.append(viewMFCC)
data_test.append(viewChroma)
model = wgcca(12)
model.fit(data_train)
t = model.transform(data_test)
t_mean = np.mean(np.array([t[0], t[1]]), axis=0)
path_csv = "gcca_features.csv"
t_final = np.transpose(t_mean)
np.savetxt(path_csv, t_final, delimiter=',')

tesst = np.loadtxt("gcca_features.csv", delimiter=',')
print(tesst.shape)
